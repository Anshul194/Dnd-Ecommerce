...
export async function POST(request) {
  try {
    ...
    if (redisWrapper.isEnabled()) {
      const storedUserId = await redisWrapper.get(`refreshToken:${refreshToken}`);
    ...
    if (redisWrapper.isEnabled()) {
      ...
      await redisWrapper.setex(
        `accessToken:${newAccessToken}`, 
        Math.floor((newAccessTokenExp - Date.now()) / 1000), 
        "valid"
      );
      await redisWrapper.setex(
        `refreshToken:${newRefreshToken}`, 
        Math.floor((newRefreshTokenExp - Date.now()) / 1000), 
        user._id.toString()
      );
    }
    ...
    Token.setTokensCookies(response, newAccessToken, newRefreshToken, newAccessTokenExp, newRefreshTokenExp);
